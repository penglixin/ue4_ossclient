...
DEFINE_LOG_CATEGORY_STATIC(LogSimpleOSS, Log, All);
...
if (bVisitSuccess) \
{\
	OSS_SUCCESS_MSG("ResponseCode[%i] ResponseCodeChar[%s] ReqID[%s] ResponseMsg[%s]  ", resp_status->code, resp_status->error_code, resp_status->error_msg, resp_status->req_id)\
}\
else \
{\
